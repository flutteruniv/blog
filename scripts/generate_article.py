...
2. **カテゴリ分類**: ニュースを「Flutter・クロスプラットフォーム開発」「React Native開発」「Swift・iOS開発」「Kotlin・Android開発」「生成AI」「開発者向け情報」「その他技術トピック」などに分類（カテゴリは「##」を使用）
...
4. **記事解説**: 各ニュースについて3〜4文程度の簡潔で具体的な解説を追記する。以下の要素を含める：
   - **技術的な詳細**: 何がどのように変更・改善されたのか（CHANGELOGなど実際の内容がある場合は具体的に要約）
   - **開発者への影響**: 実際の開発にどう影響するのか
   - **注目ポイント**: なぜこのニュースが重要なのか
   
   **重要**: 以下のような陳腐で一般的な締めくくり文は絶対に使用しない：
   - 「〜に大きく貢献します」「〜が期待されます」「〜することが可能です」
   - 「安定性向上」「パフォーマンス向上」「継続的に期待」
   - 「開発者は〜を確認し」「〜を検討することで」
   - 「より良い〜を行うことができます」
   
   代わりに、具体的な技術的洞察や実用的な情報で文章を終える。
5. **まとめセクション**: 最後に「## まとめ」セクションを追加し、今週紹介した技術の中で**最も重要なトレンドを1つ選んで**具体的に言及し、それが**モバイル開発にどのような変化をもたらすか**を述べる。汎用的な文章（「様々な話題」「目覚ましい進歩」「学習し続ける必要性」等）は避け、具体的な技術名や変更点に基づいた価値ある洞察を提供する
...
---
title: "[上記の指示に従って動的に生成]"
slug: "mobile-news-{today_str.replace('-', '')}"
author: "kboy"
description: "[上記の指示に従って動的に生成]"
